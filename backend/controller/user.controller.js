  ...
  try {
    ...
    const user = await User.findByIdAndUpdate(
      req.user._id,
      {
        ...
        country: country?.trim() || "",
        bio: bio?.trim() || "",
        // FIX: Uncommented — avatar was never being saved to DB on profile update
        ...(avatar && { avatar }),
        "socials.twitter": twitter?.trim() || "",
        "socials.github": github?.trim() || "",
        "socials.linkedin": linkedin?.trim() || "",
...
export const uploadAvatar = async (req, res) => {
  try {
    ...
    // public_id is fixed per-user + overwrite:true → only ONE asset per user
    // ever exists in Cloudinary, matching (and improving on) old behaviour.
    const uploadFromBuffer = () =>
      new Promise((resolve, reject) => {
        const stream = cloudinary.uploader.upload_stream(
          {
            folder: "webgenai/avatars",
