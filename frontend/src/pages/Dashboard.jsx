...
const MiniCreditPill = () => {
  ...
  return (
    <div
      className={`flex items-center gap-[5px] px-[10px] py-[5px] rounded-[10px] shrink-0 bg-[var(--bg-card)] border ${
        isCritical ? "border-[rgba(239,68,68,0.4)]" : "border-[var(--border)]"
      }`}
    >
...
const Dashboard = () => {
  ...
  const handleStar = async (e, id) => {
    e.stopPropagation();
    setWebsites((prev) =>
      prev.map((w) => (w._id === id ? { ...w, starred: !w.starred } : w)),
    );
    try {
      await axios.patch(
        `${serverUrl}/api/website/star/${id}`,
        {},
  ...
  const handleDelete = (e, id) => {
    ...
    toast(
      (t) => (
        <div>
          <p className="text-[13px] mb-[10px] font-medium text-[var(--text-primary)]">
            Delete this website?
          </p>
  ...
  return (
    <div className="flex flex-col h-screen overflow-hidden bg-[var(--bg-base)] text-[var(--text-primary)]">
      {/* ── Header ── */}
      <header className="sticky top-0 z-40 shrink-0 backdrop-blur-[20px] bg-[var(--bg-elevated)] border-b border-[var(--border)]">
        <div className="h-14 px-4 sm:px-6 flex items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2 shrink-0">
            ...
            <motion.button
              ...
              whileTap={{ scale: 0.97 }}
              onClick={goGenerate}
              className="flex items-center gap-1.5 px-3 sm:px-4 py-2 rounded-lg text-white text-xs sm:text-sm font-semibold whitespace-nowrap bg-gradient-to-br from-[#a855f7] to-[#3b82f6] shadow-[0_2px_12px_rgba(168,85,247,0.35)]"
            >
              <Plus size={14} />
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12">
          ...
          {!loading && !error && websites.length === 0 && (
            <motion.div
              ...
              animate={{ opacity: 1, y: 0 }}
              className="flex flex-col items-center justify-center py-20 text-center rounded-3xl bg-[rgba(255,255,255,0.025)] border border-dashed border-[var(--border)]"
            >
              <div className="w-14 h-14 rounded-2xl flex items-center justify-center mb-5 text-purple-400 bg-[rgba(168,85,247,0.1)]">
                <Sparkles size={24} />
              </div>
              <h3 className="text-lg font-semibold mb-2">No websites yet</h3>
              <p className="text-sm mb-7 max-w-xs leading-relaxed text-[var(--text-muted)]">
              ...
              <motion.button
                ...
                whileTap={{ scale: 0.97 }}
                onClick={goGenerate}
                className="flex items-center gap-2 px-6 py-3 rounded-xl text-white text-sm font-semibold bg-gradient-to-br from-[#a855f7] to-[#3b82f6]"
              >
              ...
              {websites.map((w, i) => {
                ...
                return (
                  <motion.div
                    ...
                    whileHover={{ y: -4 }}
                    className="rounded-2xl overflow-hidden flex flex-col bg-[var(--bg-card)] border border-[var(--border)]"
                  >
                    {/* Preview */}
                    <div
                    ...
                    >
                      <iframe
                        srcDoc={w.latestCode}
                        title={w.title}
                        className="absolute inset-0 w-[142%] h-[142%] pointer-events-none bg-white scale-[0.704] origin-top-left"
                        ...
                        <div className="flex items-center gap-1.5">
                          <button
                            ...
                            className={`p-1.5 rounded-md transition-all border ${
                              w.starred
                                ? "bg-[rgba(250,204,21,0.15)] border-[rgba(250,204,21,0.4)]"
                          ...
                          >
                            <Star
                              size={14}
                              fill={w.starred ? "#facc15" : "none"}
                              color={w.starred ? "#facc15" : "#9ca3af"}
                            />
                          </button>
                          <button
                            onClick={(e) => handleDelete(e, w._id)}
                            className="p-1.5 rounded-md transition-all bg-[rgba(255,255,255,0.04)] border border-[var(--border)] text-[#f87171]"
                            aria-label="Delete"
                          >
                            <Trash2 size={14} />
                          </button>
                      ...
                      <div className="mt-auto">
                        {!w.deployed ? (
                          <motion.button
                            ...
                            className="w-full flex items-center justify-center gap-2 px-4 py-2 rounded-xl text-sm font-semibold text-white bg-gradient-to-br from-[#6366f1] to-[#a855f7]"
                          >
