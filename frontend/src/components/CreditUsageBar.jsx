...
const CreditUsageBar = ({ compact = false }) => {
  ...
  // ✅ Flash animation jab credits cut hon
  useEffect(() => {
  ...
  const creditColor = isCritical
    ? "#ef4444"
    : isLow
      ? "#f97316"
      : "var(--text-primary)";

  // ✅ COMPACT — header mein dikhne wali pill
  if (compact) {
    return (
      <motion.div
        initial={{ opacity: 0 }}
        ...
        style={{
          background: flash
            ? "rgba(239,68,68,0.12)"
            : "var(--bg-card)",
          border: `1px solid ${flash ? "rgba(239,68,68,0.4)" : "var(--border)"}`,
      ...
      >
        ...
        <motion.span
          ...
          style={{ color: creditColor, minWidth: 24 }}
        >
          {remaining}
          <span
  ...
  // ✅ FULL — dashboard sidebar ya card mein
  return (
    <motion.div
      ...
      style={{
        background: flash ? "rgba(239,68,68,0.06)" : "var(--bg-card)",
        border: `1px solid ${flash ? "rgba(239,68,68,0.35)" : "var(--border)"}`,
        transition: "background 0.3s, border 0.3s",
        boxShadow: isLow
          ? `0 0 0 1px ${isCritical ? "rgba(239,68,68,0.3)" : "rgba(249,115,22,0.25)"}`
          : "none",
      }}
    >
      {/* Header Row */}
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          <div
            ...
            style={{
              background: isLow ? "rgba(249,115,22,0.1)" : "var(--accent-glow)",
            }}
          >
            {isLow ? (
        ...
        <button
          ...
          className="flex items-center gap-1 text-[11px] font-semibold px-2.5 py-1 rounded-lg transition-all duration-150"
          style={{
